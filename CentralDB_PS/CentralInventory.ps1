...
#####################################################################################################################################
#GetRSNameSpace
function GetRSNameSpace ($RSVersion, $InstanceName)
{ 
	if($RSVersion -Like '9.*'){ $rs_namespace = 'root\Microsoft\SqlServer\ReportServer\v9' } 
	elseif ($RSVersion -Like '10.*') { $rs_namespace = "root\Microsoft\SqlServer\ReportServer\RS_" + $InstanceName + "\v10" }
	elseif ($RSVersion -Like '11.0*') { $rs_namespace = "root\Microsoft\SqlServer\ReportServer\RS_" + $InstanceName + "\v11" }
	elseif ($RSVersion -Like '12.0*') { $rs_namespace = "root\Microsoft\SqlServer\ReportServer\RS_" + $InstanceName + "\v12" }
	elseif ($RSVersion -Like '13.0*') { $rs_namespace = "root\Microsoft\SqlServer\ReportServer\RS_" + $InstanceName + "\v13" }
	elseif ($RSVersion -Like '14.0*') { $rs_namespace = "root\Microsoft\SqlServer\ReportServer\RS_" + $InstanceName + "\v14" }
    return $rs_namespace
} #GetRSNameSpace
#####################################################################################################################################
...
#####################################################################################################################################
#GetRSNameSpaceAdmin
function GetRSNameSpaceAdmin ($RSVersion, $InstanceName)
{ 
	if($RSVersion -Like '9.*'){ $rs_namespace = 'root\Microsoft\SqlServer\ReportServer\v9\Admin' } 
	elseif ($RSVersion -Like '10.*') { $rs_namespace = "root\Microsoft\SqlServer\ReportServer\RS_" + $InstanceName + "\v10\Admin" }
	elseif ($RSVersion -Like '11.0*') { $rs_namespace = "root\Microsoft\SqlServer\ReportServer\RS_" + $InstanceName + "\v11\Admin" }
	elseif ($RSVersion -Like '12.0*') { $rs_namespace = "root\Microsoft\SqlServer\ReportServer\RS_" + $InstanceName + "\v12\Admin" }
	elseif ($RSVersion -Like '13.0*') { $rs_namespace = "root\Microsoft\SqlServer\ReportServer\RS_" + $InstanceName + "\v13\Admin" }
	elseif ($RSVersion -Like '14.0*') { $rs_namespace = "root\Microsoft\SqlServer\ReportServer\RS_" + $InstanceName + "\v14\Admin" }
    return $rs_namespace
} #GetRSNameSpaceAdmin
#####################################################################################################################################
...
{ 
	...
	if ($Version -Like '14.0.2*')  {$IsUpToDate = 'True'}
    return $IsUpToDate;
} #GetIsUpToDate
#####################################################################################################################################
...
function GetServerListInfo($svr, $inst) 
{
	...
	{ 
		$ex = $_.Exception 
		write-log -Message "$ex.Message on $svr while collecting Operating System Info" -Level Error -NoConsoleOut -Path $logPath 
	}
	finally
	{
	...
	{ 
			$ex = $_.Exception 
			write-log -Message "$ex.Message on $svr while collecting Page File Usage Info" -Level Error -NoConsoleOut -Path $logPath
	}
	finally
	{
		$ErrorActionPreference = "Continue"; #Reset the error action pref to default
...
‘@
	...
	{ 
			$ex = $_.Exception 
			write-log -Message "$ex.Message on $svr while collecting Server Info" -Level Error -NoConsoleOut -Path $logPath
	}
	finally
	{
		$ErrorActionPreference = "Continue"; #Reset the error action pref to default
	...
	{ 
			$ex = $_.Exception 
			write-log -Message "$ex.Message on $svr while collecting Disk and Mountpoint Info" -Level Error -NoConsoleOut -Path $logPath
	}
	finally
	{
		$ErrorActionPreference = "Continue"; #Reset the error action pref to default
	...
	{ 
			$ex = $_.Exception 
			write-log -Message "$ex.Message on $svr while collecting SQL Services Info" -Level Error -NoConsoleOut -Path $logPath
	}
	finally
	{
		$ErrorActionPreference = "Continue"; #Reset the error action pref to default
	...
	If ($responds) 
	{
			...
			try 
			{
				...
				$ip = (Get-NetIPConfiguration -ComputerName $svr).IPv4Address.IPAddress -join ','
			...
			{ 
					$ex = $_.Exception 
					write-log -Message "$ex.Message on $svr while collecting Instance Info" -Level Error -NoConsoleOut -Path $logPath
			}
			finally
			{
				$ErrorActionPreference = "Continue"; #Reset the error action pref to default
			...
			try 
			{
				...
				$dt= $dbs | select @{n="ServerName";e={$svr}}, @{n="InstanceName";e={$inst}}, name, Description, OwnerLoginName, IsEnabled, category, DateCreated, DateLastModified,  LastRunDate, NextRunDate, LastRunOutcome, CurrentRunRetryAttempt, OperatorToEmail, OperatorToPage, HasSchedule, @{n="DateAdded";e={$RunDt}} | out-datatable
				Write-DataTable -ServerInstance $InstanceName -Database $DatabaseName -TableName $CITbl -Data $dt
				Write-Log -Message "Collecting Job Info" -Level Info -Path $logPath
			}
			catch 
			{ 
					$ex = $_.Exception 
					write-log -Message "$ex.Message on $svr While Collecting Job Info" -Level Error -NoConsoleOut -Path $logPath
			}
			finally
			{
				$ErrorActionPreference = "Continue"; #Reset the error action pref to default
			...
			try 
			{
				...
				Write-DataTable -ServerInstance $InstanceName -Database $DatabaseName -TableName $CITbl -Data $dt
				Write-Log -Message "Collecting Failed Job Info" -Level Info -Path $logPath
			}
			catch 
			{ 
					$ex = $_.Exception 
					write-log -Message "$ex.Message on $svr While Collecting Failed Jobs  Info" -Level Error -NoConsoleOut -Path $logPath
			}
			finally
			{
				$ErrorActionPreference = "Continue"; #Reset the error action pref to default
			...
			try 
			{
				...
				$s = New-Object ('Microsoft.SqlServer.Management.Smo.Server') $inst
				$dbs=$s.Logins
				$dt= $dbs | SELECT @{n="ServerName";e={$svr}}, @{n="InstanceName";e={$inst}}, Name, LoginType, CreateDate, DateLastModified, IsDisabled, IsLocked, @{n="DateAdded";e={$RunDt}} |out-datatable
				Write-DataTable -ServerInstance $InstanceName -Database $DatabaseName -TableName $CITbl -Data $dt
			...
			{ 
					$ex = $_.Exception 
					write-log -Message "$ex.Message on $svr While Collecting Login Info" -Level Error -NoConsoleOut -Path $logPath
			}
			finally
			{
				$ErrorActionPreference = "Continue"; #Reset the error action pref to default
			...
			{ 
					$ex = $_.Exception 
					write-log -Message "$ex.Message on $svr While Collecting Instance Roles Info" -Level Error -NoConsoleOut -Path $logPath
			}
			finally
			{
				$ErrorActionPreference = "Continue"; #Reset the error action pref to default
			...
			try 
			{
				...
				$s = New-Object ('Microsoft.SqlServer.Management.Smo.Server') $inst
				$dbs=$s.linkedservers
				$dt= $dbs | SELECT @{n="ServerName";e={$svr}}, @{n="InstanceName";e={$inst}}, Name, ProviderName, ProductName, ProviderString, DateLastModified, DataAccess, @{n="DateAdded";e={$RunDt}} |out-datatable
				Write-DataTable -ServerInstance $InstanceName -Database $DatabaseName -TableName $CITbl -Data $dt
			...
			{ 
					$ex = $_.Exception 
					write-log -Message "$ex.Message on $svr While Collecting Linked Servers Info" -Level Error -NoConsoleOut -Path $logPath
			}
			finally
			{
				$ErrorActionPreference = "Continue"; #Reset the error action pref to default
			...
			{ 
					$ex = $_.Exception 
					write-log -Message "$ex.Message on $svr While Collecting Instance Level Triggers Info" -Level Error -NoConsoleOut -Path $logPath
			}
			finally
			{
				$ErrorActionPreference = "Continue"; #Reset the error action pref to default
			...
			{ 
					$ex = $_.Exception 
					write-log -Message "$ex.Message on $svr While Collecting Replication Publisher Info" -Level Error -NoConsoleOut -Path $logPath
			}
			finally
			{
				$ErrorActionPreference = "Continue"; #Reset the error action pref to default
			...
			{ 
					$ex = $_.Exception 
					write-log -Message "$ex.Message on $svr While Collecting Database Info" -Level Error -NoConsoleOut -Path $logPath
			}
			finally
			{
				$ErrorActionPreference = "Continue"; #Reset the error action pref to default
			...
			{ 
					$ex = $_.Exception 
					write-log -Message "$ex.Message on $svr While Collecting Availability Groups Info" -Level Error -NoConsoleOut -Path $logPath
			}
			finally
			{
				$ErrorActionPreference = "Continue"; #Reset the error action pref to default
			...
			{ 
					$ex = $_.Exception 
					write-log -Message "$ex.Message on $svr While Collecting Availability Databases Info" -Level Error -NoConsoleOut -Path $logPath
			}
			finally
			{
				$ErrorActionPreference = "Continue"; #Reset the error action pref to default
			...
			{ 
					$ex = $_.Exception 
					write-log -Message "$ex.Message on $svr While Collecting Availability Replicas Info" -Level Error -NoConsoleOut -Path $logPath
			}
			finally
			{
				$ErrorActionPreference = "Continue"; #Reset the error action pref to default
			...
			{ 
					$ex = $_.Exception 
					write-log -Message "$ex.Message on $svr While Collecting DB Level Trigger Info" -Level Error -NoConsoleOut -Path $logPath
			}
			finally
			{
				$ErrorActionPreference = "Continue"; #Reset the error action pref to default
			...
			{ 
					$ex = $_.Exception 
					write-log -Message "$ex.Message on $svr While Collecting DB user roles Info" -Level Error -NoConsoleOut -Path $logPath
			}
			finally
			{
				$ErrorActionPreference = "Continue"; #Reset the error action pref to default
			...
			try 
			{
				...
				write-log -Message "Updated Backupset Rows Affected ($rowsAffected)" -Level Info -Path $logPath
			}
			catch 
			{ 
					$ex = $_.Exception 
					write-log -Message "$ex.Message on $svr While Collecting Database Backup Info" -Level Error -NoConsoleOut -Path $logPath
			}
			finally
			{
				$ErrorActionPreference = "Continue"; #Reset the error action pref to default
			...
			try 
			{
				...
				$da.fill($dt) | out-null
				Write-DataTable -ServerInstance $InstanceName -Database $DatabaseName -TableName $CITbl -Data $dt
				Write-Log -Message "Collecting DB Files Info" -Level Info -Path $logPath
			}
			catch 
			{ 
					$ex = $_.Exception 
					write-log -Message "$ex.Message on $svr While Collecting DB Files Info" -Level Error -NoConsoleOut -Path $logPath
			}
			finally
			{
				$ErrorActionPreference = "Continue"; #Reset the error action pref to default
			...
			try 
			{
				...
				$da.fill($dt) | out-null
				Write-DataTable -ServerInstance $InstanceName -Database $DatabaseName -TableName $CITbl -Data $dt
				Write-Log -Message "Collecting DB Growth Info" -Level Info -Path $logPath
			}
			catch 
			{ 
					$ex = $_.Exception 
					write-log -Message "$ex.Message on $svr While Collecting DB Growth Info" -Level Error -NoConsoleOut -Path $logPath
			}
			finally
			{
				$ErrorActionPreference = "Continue"; #Reset the error action pref to default
			...
			{ 
				$ex = $_.Exception 
				write-log -Message "$ex.Message on $svr While Collecting Table Permissions Info" -Level Error -NoConsoleOut -Path $logPath
			}
			finally
			{
				$ErrorActionPreference = "Continue"; #Reset the error action pref to default
			...
			try 
			{
				...
				$da.fill($dt) | out-null
				Write-DataTable -ServerInstance $InstanceName -Database $DatabaseName -TableName $CITbl -Data $dt
				Write-Log -Message "Collecting Memory (Hekaton) Tables Info" -Level Info -Path $logPath
			}
			catch 
			{ 
					$ex = $_.Exception 
					write-log -Message "$ex.Message on $svr While Collecting Memory (Hekaton) Tables Info" -Level Error -NoConsoleOut -Path $logPath
			}
			finally
			{
				$ErrorActionPreference = "Continue"; #Reset the error action pref to default
	...
	try 
	{
		...
		if ($responds) 
		{
			...
			$rs_namespace = GetRSNameSpace $s.Version $instname
			...
			Write-DataTable -ServerInstance $InstanceName -Database $DatabaseName -TableName $CITbl -Data $dt
			Write-Log -Message "Collecting RS Info" -Level Info -Path $logPath

			$CITbl="[RS].[SSRSConfig]"
			#$SQLServerConnection = $inst
			$s = New-Object ('Microsoft.SqlServer.Management.Smo.Server') $inst
			$rs_namespace = GetRSNameSpaceAdmin $s.Version $instname
			...
			Write-DataTable -ServerInstance $InstanceName -Database $DatabaseName -TableName $CITbl -Data $dt
			Write-Log -Message "Collecting RS Config Info" -Level Info -Path $logPath
		}
		else
		{
			Write-log -Message "Reporting Services is not Installed or Started or inaccessible on $inst"  -NoConsoleOut -Path $logPath
		}
	...
	{ 
			$ex = $_.Exception 
			write-log -Message "$ex.Message on $svr While Collecting RS Info" -Level Error -NoConsoleOut -Path $logPath
	}
	finally
	{
		$ErrorActionPreference = "Continue"; #Reset the error action pref to default
	...
	try 
	{
		...
		if ($responds) 
		{
			...
			$CITbl="[AS].[SSASInfo]"
			$dt =  $s | Select @{n="ServerName";e={$svr}}, @{n="InstanceName";e={$inst}}, ProductName, @{n="SQLASVersion";e={$SQLASVersion}}, ProductLevel, @{n="IsSPUpToDateOnAS";e={$IsSPUpToDateOnAS}},@{n="SQLASEdition";e={$SQLASEdition}}, Version, @{n="NoOfDBs";e={($_.Databases.Count)}}, LastSchemaUpdate, Connected, IsLoaded, @{n="DateAdded";e={$RunDt}} | out-datatable
			Write-DataTable -ServerInstance $InstanceName -Database $DatabaseName -TableName $CITbl -Data $dt

			$CITbl="[AS].[SSASDBInfo]"
			$dt =  $s.Databases | Select @{n="ServerName";e={$svr}}, @{n="InstanceName";e={$inst}},  Name, @{Expression={$_.EstimatedSize / 1MB};Label="DBSizeInMB"}, Collation, CompatibilityLevel, CreatedTimestamp, LastProcessed, LastUpdate, DBStorageLocation, @{n="NoOfCubes";e={($_.Cubes.Count)}},  @{n="NoOfDimensions";e={($_.Dimensions.Count)}}, ReadWriteMode, StorageEngineUsed, Visible, @{n="DateAdded";e={$RunDt}} | out-datatable
			Write-DataTable -ServerInstance $InstanceName -Database $DatabaseName -TableName $CITbl -Data $dt

		}
		else
		{
			Write-log -Message "Analysis Services is not Installed or Started or inaccessible on $svr" -NoConsoleOut -Path $logPath
		}
	}
	catch 
	{ 
			$ex = $_.Exception 
			write-log -Message "$ex.Message on $svr While Collecting AS Info" -Level Error -NoConsoleOut -Path $logPath
	}
	finally
	{
		$ErrorActionPreference = "Continue"; #Reset the error action pref to default
