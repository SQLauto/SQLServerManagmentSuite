...
param(
    ...
    [string]$Path    = "\\" + ((ipconfig | select-string -notmatch "169.254" | select-string -notmatch "169.168" | select-string -notmatch "255.255" | select-string -notmatch "192.168" | findstr [0-9].\.)[0].Split()[-1]).ToString().Remove(((ipconfig | findstr [0-9].\.)[0].Split()[-1]).ToString().LastIndexOf(".")) + ".195" + "\SQL_NASBackup",
    [string]$ErrorPath = "\\" + ((ipconfig | select-string -notmatch "169.254" | select-string -notmatch "169.168" | select-string -notmatch "255.255" | select-string -notmatch "192.168" | findstr [0-9].\.)[0].Split()[-1]).ToString().Remove(((ipconfig | findstr [0-9].\.)[0].Split()[-1]).ToString().LastIndexOf('.')) + ".195" + "\SQL_NASBackup\_CentralDB\Errorlog\"
	)
...
{ 
   ...
   try 
    { 
        ...
        #$bulkCopy
        #write-log -Message "connectionString" -Path $ErrorPath
        $bulkCopy.DestinationTableName = $tableName 
        #write-log -Message "TableName $tableName" -Path $ErrorPath
        $bulkCopy.BatchSize = $BatchSize 
        $bulkCopy.BulkCopyTimeout = $QueryTimeOut 
        $bulkCopy.WriteToServer($Data) 
        #write-log -Message "write to server" -Path $ErrorPath
        $conn.Close() 
    } 
...
try 
    { 	
        ...
        write-log -Message "$Type | Start Path $Path Processing"
        cd C:\Windows
        $Files = cmd.exe /c "dir $Path /a-d /-c /s"
        write-log -Message "$Type | Finished Path $Path Processing"
        return ,$Files
    }    
catch 
	{ 
        $ex = $_.Exception 
	    write-log -Message "$Type | Generated this exception [$ex] while collecting backup file data" -Path $ErrorPath
	} 
finally{
...
try 
    { 
        ...
        write-log -Message "$Type | Starting DataTable Processing" -Path $ErrorPath      
        ...
        write-log -Message "$Type | Finished DataTable Processing" -Path $ErrorPath   

        $RowCount = $dt.Rows.Count.ToString()
        #write-log -Message "RowCount $RowCount"  -Path $ErrorPath
        if($dt.Rows.Count -gt "0")
        {
            write-log -Message "$Type | Starting Bulk Load to $SQLInst on $Centraldb in $CITbl" -Path $ErrorPath 
            Write-DataTable -ServerInstance $SQLInst -Database $Centraldb -TableName $CITbl -Data $dt
            write-log -Message "$Type | Finished Bulk Load to $SQLInst on $Centraldb in $CITbl" -Path $ErrorPath            
        }
        else
        {
            write-log -Message "$Type | On $SQLInst, there are no rows in the table or table does not exist."  -Path $ErrorPath
        }
    }    
catch 
	{ 
        $ex = $_.Exception 
	    write-log -Message "$Type | $ex.Message on $server While executing maintenance solution"  -Path $ErrorPath
	} 
finally
    {
...
######################################################################################################################################
 ...
 $origErrorPath = $ErrorPath
 $ErrorPath = $ErrorPath  + "CollectBackupFileData.log"
 if($Type.Length -ne 0)
 {  
    $server = $env:computername
	$ErrorPath = $origErrorPath  +  $server + "_CollectBackupFileData.log"
	$ElapsedTime = [System.Diagnostics.Stopwatch]::StartNew()
		
	write-log -Message "$Type | Script Started at $(get-date)" -Path $ErrorPath

    # Calling funtion and pass type and path
	$GUID = [guid]::NewGuid().Tostring()
	$Files = CollectBackupFileData $Type $Path
    if ($Files -ne $null)
    {
...
write-log -Message "$Type | Script Ended at $(get-date)"  -Path $ErrorPath
write-log -Message "$Type | Total Elapsed Time: $($ElapsedTime.Elapsed.ToString())"  -Path $ErrorPath
write-log -Message "----------------------------------" -Path $ErrorPath	